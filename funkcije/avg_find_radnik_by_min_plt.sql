CREATE OR REPLACE FUNCTION avg_find_radnik_by_min_plt (
	rbt IN NUMBER, 
...
BEGIN
	...
	BEGIN
		...
		LOOP
			upit := 'SELECT min(plt) FROM radnik_' || rbt;
			write_master_log(upit, 'avg_find_radnik_by_min_plt.log');
...
END avg_find_radnik_by_min_plt; 
/
...
BEGIN
	FOR iter IN 1..5
	LOOP
		dbms_output.put_line('Prosecno vreme odziva, radnik_' || (iter * 4) || ' je: ' || avg_find_radnik_by_min_plt(iter * 4, 3, 2));
	END LOOP; 
END; 
/